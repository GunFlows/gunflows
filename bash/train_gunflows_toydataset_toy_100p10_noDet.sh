...
srun --ntasks=1 apptainer exec --nv   --env PYTHONNOUSERSITE=1   --env PYTHONPATH="/workspace/work/GuNFlows/src:/workspace/work/GuNFlows/src/normalizing-flows"   --bind "${HOST_REPO}:${IN_CONTAINER_WORKDIR}"   --bind "${HOST_CONFIG}:/workspace/config"   --bind "${HOST_DATA}:/workspace/data"   --pwd "${IN_CONTAINER_WORKDIR}"   "${SIF}" bash -lc "source '${IN_CONTAINER_SETUP}' &&                      HYDRA_FULL_ERROR=1 python -s -m gunflows.train experiment=toyOA_100p10_noDet_toy ${EXTRA_ARGS}"
