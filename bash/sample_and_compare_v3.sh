...
# Like sample_and_compare_v2.sh but also binds common_gundam_workspace
# so MCMC files stored there are accessible at /workspace/common_workspace.
# Use when: NF trained against ToyNDFit (postfit ROOT in ToyNDFit/GundamWorkspace/output/)
#           and MCMC chain lives in common_gundam_workspace/.
...
HOST_COMMON="/home/shares/sanchezf/gundam_n_flow/common_gundam_workspace"
...
srun --ntasks=1 apptainer exec --nv \
  ...
  --bind "${HOST_COMMON}:/workspace/common_workspace" \
