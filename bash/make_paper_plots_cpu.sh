#!/bin/bash
#SBATCH --job-name=paper_plots_cpu
...
# CPU-only variant for PLOT-ONLY runs (force_recompute=false on a complete
# cache): no GUNDAM/NF, so no GPU needed — runs on shared-cpu.
