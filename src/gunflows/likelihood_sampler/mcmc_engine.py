...
Generic parallel MCMC engine over any likelihood-sampler backend (see base.py
for the required interface). Resolves the concrete backend at runtime from
`cfg.likelihood.sampler_target` -- this module never imports one by name.

Two modes (set via config ``mode``):

  parallel_tempering  (default)
                N workers at temperatures beta_1=1 > beta_2 > ... > beta_n run
                in parallel. Hot chains explore; the cold chain (beta=1)
                samples the target. Swaps attempted every swap_interval steps.

  independent   N chains run fully in parallel with different seeds, then
                merged. Zero communication; wall-clock ~= 1 chain.

Both modes use Cholesky-preconditioned proposals:
    x' = x + sigma * L @ z,   z ~ N(0,I),   L = chol(postfit_cov)
...
def _make_sampler(llh_cfg):
    LikelihoodSampler = resolve_target(str(llh_cfg.sampler_target))
    cwd = llh_cfg.get("cwd", None)
    with pushd(cwd or os.path.dirname(str(llh_cfg.config))):
...
def _worker(rank: int,
    ...
    """
    Owns one likelihood-sampler backend.
...
def run_mcmc(cfg: DictConfig, base_dir: str) -> None:
    """Dispatch to the parallel-tempering or independent-chains coordinator."""
