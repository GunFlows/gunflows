...
import multiprocessing as mp, queue as _q, os
...
class NFSamplerProcess(mp.Process):
    ...
    def _load_nf(self):
        nf = torch.load(self.nf_ckpt, map_location="cpu")
        nf.eval()
        return nf

    def _load_llh(self):
        with pushd(self.llh_cwd):
            return LikelihoodSampler(self.llh_config,
                                     override_files=self.llh_overrides,
                                     threads=1, data_is_asimov=True, seed=self.seed)
    ...
    def run(self):
        ...
        eps = 1e-6
        L = torch.linalg.cholesky(cov_t + eps * torch.eye(cov_t.shape[0], dtype=cov_t.dtype))

        use_nf = self.nf_ckpt is not None
        nf = self._load_nf() if use_nf else None

        while not self.stop_evt.is_set():
            if use_nf:
                z = torch.randn(self.n_points, nf.latent_dim, generator=rng)
                x_std, _ = nf.inverse(z)
