...
def _plot_grid(samples, mean, weights, cov, names, n, out_dir, phase_dims=None, tag="grid", stage=0):
    ...
    fig.suptitle(f"stage {stage}", fontsize=9)
    plt.tight_layout()
    plt.savefig(out_dir / f"grid_{tag}.png", dpi=150)
    plt.close(fig)


class StreamingDataset(BaseSystematicDataset):
    def __init__(
        self,
        phase_space_dim,
        ...
        threads=10,
        num_samplers=1,
        sampler_modes=None,                # e.g. ["auto", "auto", "mcmc"]; cycled across num_samplers workers. None -> every worker uses "auto" (unchanged COV/NF behavior).
        data_is_asimov=True,
        model_cfg=None,
        max_batches=10,
        ...
        if with_sampler:
            ...
            if sampler_target is None:
                raise ValueError(
                    "sampler_target must be provided when with_sampler=True "
                    "(dotted path to a likelihood-sampler class, e.g. "
                    "apps.gundam.likelihoodSampler.LikelihoodSampler)"
                )
            self._start_sampler(
                nf_ckpt=nf_ckpt,
    ...
    def refresh_if_ready(self, plot_grid=True):
        ...
        if self._data_q is not None:
            ...
            if added:
                self.stage += 1
                do_plot = plot_grid and (self.stage % self.plot_every == 0)
                self._rebuild_merged(plot_grid=do_plot)
                updated = True
        return updated
    ...
    def _append_batch_path(self, path, tag):
        ...
        evicted = self._batches[0] if len(self._batches) == self._batches.maxlen else None
        self._batches.append((path, tag))
        if evicted is not None:
            evicted_path, evicted_tag = evicted
            if evicted_tag != "starting":
                try:
    ...
    def _rebuild_merged(self, plot_grid=True):
        ...
        data_std_list, log_p_list, log_q_list, is_mcmc_list = [], [], [], []
        meta_last = None
        ...
        for path, tag in list(self._batches):
            try:
                ...
                self._log(f"Batch no longer on disk, skipping: {path}")
                try:
                    self._batches.remove((path, tag))
                except ValueError:
                    pass
                self._seen_paths.discard(os.path.realpath(path))
                continue
            ...
            is_mcmc = ("mode" in ld) and (str(ld["mode"]) == "MCMC")
            if is_mcmc:
                L_phys_anchor = torch.linalg.cholesky(cov_ref)
                diff_phys = (data_phys - mean_ref).T
                y_phys = torch.linalg.solve_triangular(L_phys_anchor, diff_phys, upper=False)
                quad_phys = (y_phys * y_phys).sum(dim=0)
                const_phys = data_phys.shape[1] * np.log(2.0 * np.pi)
                logdet_cov_phys = 2.0 * torch.log(torch.diag(L_phys_anchor)).sum()
                log_q_anchor = 0.5 * (quad_phys + const_phys + logdet_cov_phys)
            ...
            is_mcmc_list.append(torch.full((data_std.shape[0],), is_mcmc, dtype=torch.bool))
        ...
        if not data_std_list:
            self._log("All tracked batches were missing on disk; keeping previous data.")
            return
        ...
        non_mcmc = ~self.is_mcmc.reshape(-1)

        if plot_grid and bool(non_mcmc.any()):
            logw = (self.log_q - self.log_p).reshape(-1)
            q = torch.quantile(logw[non_mcmc], 0.99)
            mask = non_mcmc & (logw <= q)
            ...
            _plot_grid(
                ...
                weights_np,
                cov_phys,
                self.titles, min(10,len(self.phase_space_dim)), self.out_dir, self.phase_space_dim, "weighted", self.stage,
            )
            _plot_grid(
                samples_phys,
                mean_phys,
                ...
                cov_phys,
                self.titles, min(10,len(self.phase_space_dim)), self.out_dir, self.phase_space_dim, "unweighted", self.stage,
            )
            ...
            if latest_samples_phys is not None:
                ...
                _plot_grid(
                    ...
                    np.exp(lw),
                    latest_cov_phys,
                    latest_titles, min(10,len(self.phase_space_dim)), self.out_dir, self.phase_space_dim, "latest", self.stage,
                )
                _plot_grid(
                    latest_samples_phys[m_last, :],
                    latest_mean_phys,
    ...
    def _start_sampler(self, nf_ckpt, n_points, llh_config, llh_overrides, llh_cwd, sampler_target, seed, queue_size, save_dir=None, write_every=None, threads=6, data_is_asimov=True, model_cfg=None, num_samplers=1, sampler_modes=("auto",), keep_last_n_batches=5):
        ...
        for i in range(int(num_samplers)):
            cmd_q = mp.Queue()
            worker_mode = sampler_modes[i % len(sampler_modes)]
            p = NFSamplerProcess(
                nf_ckpt=nf_ckpt,
                n_points=n_points,
                llh_config=llh_config,
