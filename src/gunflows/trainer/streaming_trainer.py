...
# -*- coding: utf-8 -*-
"""
Title : StreamingTrainer
Author: Mathias El Baz
Date  : 2025-08-14

Schedule-driven training with periodic validation and early stopping, for
datasets that stream newly-generated batches in from a background sampler
(see gunflows.dataset.streaming_dataset.StreamingDataset).
...
class StreamingTrainer(BaseTrainer):
    def __init__(self, cfg: DictConfig, model, dataset, optimizer, scheduler, **kwargs):
        ...
        self.keep_last_n_sampler_ckpts = int(tcfg.get("keep_last_n_sampler_ckpts", 2))
        self.refresh_every = int(tcfg.get("refresh_every", 1))
    ...
    def _save_full_model_for_sampling(self, epoch: int, use_best: bool) -> Path:
        ...
        if self.save_full_nf and "model" in self.cfg:
            ...
            with open(base.with_suffix(".json"), "w") as f:
                json.dump(meta, f)
        self._prune_old_sampler_ckpts()
        return base.with_suffix(".pt")

    def _prune_old_sampler_ckpts(self) -> None:
        ckpts = sorted(self.ckpt_dir.glob("sampler_epoch*.pt"))
        for old in ckpts[:-self.keep_last_n_sampler_ckpts] if self.keep_last_n_sampler_ckpts > 0 else []:
